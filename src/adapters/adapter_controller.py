...
class AdapterController(nn.Module):
    ...
    def forward(self, inputs, task, y=None):
        """
        ...
        if self.config.use_scaling_factor:
            outputs = self.config.scaling_factor*outputs # scaling*output + input or y

        if self.add_layer_norm_after_adapter:
            outputs = self.post_layer_norm(outputs)
        if self.config.use_parallel_adapter:
            outputs = outputs + y # for ffn_parallel_adapter, input = ffn-input, output = adapter(x) + ffn-output
        else:
