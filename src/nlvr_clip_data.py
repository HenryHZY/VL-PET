...
class NLVRFineTuneDataset(Dataset):
    def __init__(self, split='train', raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train'):
        ...
        self.source_to_h5 = {
            'train': nlvr_dir.joinpath(f'clip_features/data_clip_{self.args.feature_type}_att/'),
            'valid': nlvr_dir.joinpath(f'clip_features/data_clip_{self.args.feature_type}_att/'),
            'test': nlvr_dir.joinpath(f'clip_features/data_clip_{self.args.feature_type}_att/'),
        }
    ...
    def __getitem__(self, idx):
        ...
        for key in ['img0', 'img1']:
            ...
            path = self.source_to_h5[source].joinpath(f"{img_id}.h5")
            if self.args.use_vis_noise:
                path_noise = nlvr_dir.joinpath(f'clip_features/noise_data_clip_{self.args.feature_type}_att/{img_id}.npy')
        ...
        out_dict['vis_feats'] = torch.stack(feats2)
        out_dict['boxes'] = torch.stack(boxes2)
