...
class VQAFineTuneDataset(Dataset):
    def __init__(self, split='train', raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train'):
        ...
        if self.args.vis_pooling_output:
            self.source_to_h5 = {
                _type: coco_feature_dir.joinpath(f"data_clip_{_type}_fc") 
                for _type in ["RN50", "RN101", "RN50x4", "ViT"]
            }
        else:
            self.source_to_h5 = { # default goto
                _type: coco_feature_dir.joinpath(f"data_clip_{_type}_att") 
                for _type in ["RN50", "RN101", "RN50x4", "ViT"]
            }

        self.h5_path = self.source_to_h5[self.feature_type]
    ...
    def __getitem__(self, idx):
        ...
        if self.args.use_vision:
            ...
            path = self.h5_path.joinpath(f"{img_id}.h5")
            
            if self.args.use_vis_noise:
                path_noise = coco_dir.joinpath(f'clip_features/noise_data_clip_{self.args.feature_type}_att/{img_id}.npy')
                feats = np.load(path_noise)
...
class VQAEvaluator:
    ...
    def dump_result(self, quesid2ans: dict, path):
        ...
        os.makedirs(path.replace("/karpathy_test_predict.json", ""), exist_ok=True)
