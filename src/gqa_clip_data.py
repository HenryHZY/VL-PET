...
class GQAFineTuneDataset(Dataset):
    def __init__(self, split='train,valid', raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train'):
        ...
        self.featname_to_h5 = {
            'others': vg_dir.joinpath(
                f'clip_features/data_clip_{self.args.feature_type}_att/'
            ),
            'testdev': gqa_dir.joinpath(
                f'clip_features/data_clip_{self.args.feature_type}_att/'
            ),
        }
    ...
    def __getitem__(self, idx):
        ...
        if self.args.use_vision:
            ...
            h5_path = self.featname_to_h5[featname]

            path = h5_path.joinpath(f"{img_id}.h5")
            if self.args.use_vis_noise:
                if featname == 'testdev': path_noise = gqa_dir.joinpath(f'clip_features/noise_data_clip_{self.args.feature_type}_att/{img_id}.npy')
                elif featname == 'others': path_noise = vg_dir.joinpath(f'clip_features/noise_data_clip_{self.args.feature_type}_att/{img_id}.npy')
                else:
                    print('error')
                    exit(0)
