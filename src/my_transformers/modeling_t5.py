...
class T5LayerFF(nn.Module):
    ...
    def forward(self, hidden_states, block_adapters, task):
        ...
        # elif self.ff_adapter_multihead_down is not None: #【补充：记得做上残差连接】
        elif not self.is_decoder and self.config.use_encoder_adapter_down_multihead: # 【补充：h个 768 → x/h，再加上x → 768】
            adapter_down_multihead_output = [self.ff_adapter_multihead_down[i](forwarded_states) for i in range(self.config.encoder_adapter_multihead_num_head)] # 【补充：multihead和拼接】
            ...
            if self.config.use_encoder_adapter_scaling:
                adapter_up_multihead_output = adapter_up_multihead_output*self.config.encoder_adapter_scaling_factor

            if self.config.use_encoder_x2_scaling:
                forwarded_states = forwarded_states*self.config.encoder_x2_scaling_factor

            forwarded_states = forwarded_states + adapter_up_multihead_output #【补充：残差连接，Adapter Input + LowRank Output】
        ...
        if not self.is_decoder:
            if self.config.use_encoder_adapter_gating_large_x_lowrank:
                gate = self.encoder_ff_adapter_gating_large_x_down(hidden_states)
                gate = self.gating_non_linear(gate)
                gate = self.encoder_ff_adapter_gating_large_x_up(gate)
                gate = torch.sigmoid(gate)
                forwarded_states = forwarded_states*gate
            elif self.config.use_encoder_adapter_gating_small_xy_cat:
                gate_input = torch.cat([hidden_states, forwarded_states], dim=2)
                ...
                forwarded_states = forwarded_states*gate
            elif self.config.use_encoder_adapter_gating_middle_xy_add:
                gate_input = hidden_states + forwarded_states
                gate = self.encoder_ff_adapter_gating_middle_xy_add(gate_input)
                gate = torch.sigmoid(gate)
                forwarded_states = forwarded_states*gate
            elif self.config.use_encoder_adapter_gating_middle_ia3_add:
                forwarded_states = forwarded_states + forwarded_states*self.encoder_ff_adapter_gating_middle_ia3_add

            if self.config.use_encoder_gating_scaling:
                forwarded_states = forwarded_states*self.config.encoder_gating_scaling_factor

        hidden_states = hidden_states + self.dropout(forwarded_states) #【补充：相当于residual + hidden，残差连接，FF Input + FF(Adapter) Output】
        return hidden_states
...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False,
        ...
        self.use_self_vpa = use_self_vpa
        self.use_enc_vpa = use_enc_vpa
        ...
        # self.EncDecAttention = T5Attention(config=config, has_relative_attention_bias=False, adapter_config=config.use_decoder_enc_attn_value_parallel_adapter_down_dim_config, use_enc_vpa=True)
        if self.use_self_vpa or self.use_enc_vpa:
            self.attn_value_parallel_adapter = AdapterController(adapter_config)
        else:
    ...
    def forward(
        ...
        def project_vpa(hidden_states, proj_layer, key_value_states, past_key_value):
            """ projects hidden states correctly to key/query states """
            if key_value_states is None:
                # self-attn
                # (batch_size, n_heads, seq_length, dim_per_head)
                ...
                #【补充：修改 proj → vpa → shape】
                # hidden_states = shape(proj_layer(key_value_states))
                hidden_states = proj_layer(key_value_states)
                if self.attn_value_parallel_adapter is not None:
                    hidden_states = self.attn_value_parallel_adapter(key_value_states, task, y=hidden_states)
        ...
        value_states = project_vpa(
            hidden_states, self.v, key_value_states, past_key_value[1] if past_key_value is not None else None
        )

        # print('hidden_states.shape 2= ', hidden_states.shape)
        # if key_value_states: print('key_value_states.shape = ', key_value_states.shape)
        # print('value_states.shape = ', value_states.shape)
        # exit(0)

        # hidden_states.shape =  torch.Size([208, 39, 768])
        # hidden_states.shape 2=  torch.Size([208, 39, 768])
        # value_states.shape =  torch.Size([208, 12, 39, 64])
...
class T5LayerSelfAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False, is_decoder=False):
        ...
        if config.use_decoder_self_attn_value_parallel_adapter_down_dim:
            ...
            config.use_decoder_self_attn_value_parallel_adapter_down_dim_config.use_parallel_adapter = True
            self.SelfAttention = T5Attention(config=config, has_relative_attention_bias=has_relative_attention_bias, 
    ...
    def forward(
        ...
        # elif self.attn_adapter_multihead_down is not None: #【补充：记得做上残差连接】
            ...
            adapter_down_multihead_output = [self.attn_adapter_multihead_down[i](y) for i in range(self.config.encoder_adapter_multihead_num_head)] # 【补充：multihead和拼接】
            adapter_down_multihead_output = torch.cat(adapter_down_multihead_output, dim=-1)
            ...
            if self.config.use_encoder_x2_scaling:
                y = y*self.config.encoder_x2_scaling_factor

            y = y + adapter_up_multihead_output #【补充：残差连接，Adapter Input + LowRank Output】
        ...
        if not self.is_decoder:
            if self.config.use_encoder_adapter_gating_large_x_lowrank:
                gate = self.encoder_attn_adapter_gating_large_x_down(hidden_states)
                gate = self.gating_non_linear(gate)
                gate = self.encoder_attn_adapter_gating_large_x_up(gate)
                gate = torch.sigmoid(gate)
                y = y*gate
            elif self.config.use_encoder_adapter_gating_small_xy_cat:
                gate_input = torch.cat([hidden_states, y], dim=2)
                gate = self.encoder_attn_adapter_gating_small_xy_cat(gate_input)
                gate = torch.sigmoid(gate)
                gate = torch.mean(gate, dim=1).unsqueeze(-1) # [batch,len,1] → torch.mean会[batch, 1]，所以要unsqueeze
                y = y*gate
            elif self.config.use_encoder_adapter_gating_middle_xy_add:
                gate_input = hidden_states + y
                gate = self.encoder_attn_adapter_gating_middle_xy_add(gate_input)
                gate = torch.sigmoid(gate)
                y = y*gate
            elif self.config.use_encoder_adapter_gating_middle_ia3_add:
                y = y + y*self.encoder_attn_adapter_gating_middle_ia3_add

            if self.config.use_encoder_gating_scaling:
                y = y*self.config.encoder_gating_scaling_factor

        hidden_states = hidden_states + self.dropout(y) #【补充：相当于residual + hidden，残差连接，SelfAttn Input + SelfAttn(Adapter) Output】
...
class T5LayerCrossAttention(nn.Module):
    def __init__(self, config, is_decoder=False):
        super().__init__()
        self.config = config
        if config.use_decoder_enc_attn_value_parallel_adapter_down_dim:
            ...
            config.use_decoder_enc_attn_value_parallel_adapter_down_dim_config.use_parallel_adapter = True
            self.EncDecAttention = T5Attention(config=config, has_relative_attention_bias=False, 
        ...
        # if is_decoder
        if ((config.use_adapter or config.use_compacter or config.use_lradapter) and config.add_adapter_cross_attn) and not config.no_decoder_adapter: #【补充：add_adapter_cross_attn默认为True】
...
class T5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        ...
        self.layer.append(T5LayerSelfAttention(config, has_relative_attention_bias=has_relative_attention_bias, is_decoder=self.is_decoder))
        if self.is_decoder:
            self.layer.append(T5LayerCrossAttention(config, is_decoder=self.is_decoder))

        self.layer.append(T5LayerFF(config, is_decoder=self.is_decoder))
